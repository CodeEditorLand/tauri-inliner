...
function Inliner(source, options, callback) {
	...
	Object.keys(this.jobs.tasks).forEach(
		function (key) {
...
function main() {
	...
	return fs
		...
		.then(function exists(isFile) {
			...
			return inliner.get(inliner.filename || url, { encoding: "binary" });
		})
		...
		.then(function processHTML(res) {
			...
			if (enc !== "utf-8") {
				...
				$("meta").each(function charMeta() {
					...
					if (
						attrs.charset ||
						content.toLowerCase().indexOf("charset=") !== -1
					) {
						$(this).remove();
					}
				});
...
function updateTodo() {
	...
	var breakdown = Object.keys(jobs.breakdown)
		.map(function (key) {
